...
class Assistant(Agent):
    """
    Sample use case of Livekit-Moss-Voice Agent
    """
    ...
    @function_tool
    async def search_moss(self, context: RunContext, question: str) -> str:
        """
        Retrieve relevant answers using Moss semantic search.
        This tool is called by the LLM when it detects a support question.
        """
        query = question.strip()
        if not query:
            logger.warning("Moss search called with an empty question.")
            return "No question provided for Moss search."
        ...
        if not docs:
            logger.info("No Moss matches returned for query: %s", query)
            summary = "No relevant Moss matches were found for your question."
        else:
        ...
        context.session.history.add_message(
            role="system",
            content="Moss search context for the current user request:\n" + summary,
